...
def tabla():
	...
	conn.commit()
	c.execute("""CREATE TABLE compra_has_producto (cantidad integer,precio_unitario integer,total integer,FOREIGN KEY (fk_id_compra) 	REFERENCES compra(id_compra),FOREIGN KEY(fk_id_producto) REFERENCES productos(id_producto))""")
	conn.commit()
